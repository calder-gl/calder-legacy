...
class PostfixExpression private (_source: String) {
  ...
  def this(postfixExpression: PostfixExpression, FIELD_SELECTION: FieldSelection) = this(s"${postfixExpression.source()}. ${FIELD_SELECTION.source()}")
...
class FunctionCallOrMethod private (_source: String) {
  ...
  def this(postfixExpression: PostfixExpression, functionCallGeneric: FunctionCallGeneric) = this(s"${postfixExpression.source()}. ${functionCallGeneric.source()}")
  def source() = _source
}
...
class UnaryExpression private (_source: String) {
  ...
  def this(unaryOperator: UnaryOperator, unaryExpression: UnaryExpression) = this(s"${unaryOperator.source()}${unaryExpression.source()}")
  def source() = _source
}
...
class MultiplicativeExpression private (_source: String) {
  def this(unaryExpression: UnaryExpression) = this(s"${unaryExpression.source()}")
  def this(multiplicativeExpression: MultiplicativeExpression, unaryExpression: UnaryExpression) = this(s"${multiplicativeExpression.source()}* ${unaryExpression.source()}")
  def this(multiplicativeExpression: MultiplicativeExpression, SLASH: Slash, unaryExpression: UnaryExpression) = this(s"${multiplicativeExpression.source()}/ ${unaryExpression.source()}")
  def this(multiplicativeExpression: MultiplicativeExpression, PERCENT: Percent, unaryExpression: UnaryExpression) = this(s"${multiplicativeExpression.source()}% ${unaryExpression.source()}")
  def source() = _source
}
...
class AdditiveExpression private (_source: String) {
  ...
  def this(additiveExpression: AdditiveExpression, multiplicativeExpression: MultiplicativeExpression) = this(s"${additiveExpression.source()}- ${multiplicativeExpression.source()}")
  def source() = _source
}
...
class RelationalExpression private (_source: String) {
  ...
  def this(relationalExpression: RelationalExpression, LEFT_ANGLE: LeftAngle, shiftExpression: ShiftExpression) = this(s"${relationalExpression.source()}< ${shiftExpression.source()}")
  def this(relationalExpression: RelationalExpression, RIGHT_ANGLE: RightAngle, shiftExpression: ShiftExpression) = this(s"${relationalExpression.source()}> ${shiftExpression.source()}")
  def this(relationalExpression: RelationalExpression, shiftExpression: ShiftExpression) = this(s"${relationalExpression.source()}<= ${shiftExpression.source()}")
  def this(relationalExpression: RelationalExpression, GE_OP: GeOp, shiftExpression: ShiftExpression) = this(s"${relationalExpression.source()}>= ${shiftExpression.source()}")
  def source() = _source
}
...
class EqualityExpression private (_source: String) {
  ...
  def this(equalityExpression: EqualityExpression, NE_OP: NeOp, relationalExpression: RelationalExpression) = this(s"${equalityExpression.source()}!= ${relationalExpression.source()}")
  def source() = _source
}
...
class ExclusiveOrExpression private (_source: String) {
  ...
  def this(exclusiveOrExpression: ExclusiveOrExpression, andExpression: AndExpression) = this(s"${exclusiveOrExpression.source()}^ ${andExpression.source()}")
  def source() = _source
}
...
class LogicalAndExpression private (_source: String) {
  ...
  def this(logicalAndExpression: LogicalAndExpression, inclusiveOrExpression: InclusiveOrExpression) = this(s"${logicalAndExpression.source()}&& ${inclusiveOrExpression.source()}")
  def source() = _source
}
...
class LogicalOrExpression private (_source: String) {
  ...
  def this(logicalOrExpression: LogicalOrExpression, logicalXorExpression: LogicalXorExpression) = this(s"${logicalOrExpression.source()}|| ${logicalXorExpression.source()}")
  def source() = _source
}
...
class Declaration private (_source: String) {
  ...
  def this(precisionQualifier: PrecisionQualifier, typeSpecifierNoPrec: TypeSpecifierNoPrec) = this(s"precision ${precisionQualifier.source()}${typeSpecifierNoPrec.source()}; ")
  def this(typeQualifier: TypeQualifier, IDENTIFIER: Identifier, structDeclarationList: StructDeclarationList) = this(s"${typeQualifier.source()}${IDENTIFIER.source()}{ ${structDeclarationList.source()}} ; ")
  def this(typeQualifier: TypeQualifier, IDENTIFIER: Identifier, structDeclarationList: StructDeclarationList, IDENTIFIER2: Identifier) = this(s"${typeQualifier.source()}${IDENTIFIER.source()}{ ${structDeclarationList.source()}} ${IDENTIFIER2.source()}; ")
  def this(typeQualifier: TypeQualifier, IDENTIFIER: Identifier, structDeclarationList: StructDeclarationList, IDENTIFIER2: Identifier, constantExpression: ConstantExpression) = this(s"${typeQualifier.source()}${IDENTIFIER.source()}{ ${structDeclarationList.source()}} ${IDENTIFIER2.source()}[ ${constantExpression.source()}] ; ")
...
class FunctionDeclarator private (_source: String) {
  ...
  def this(functionHeaderWithParameters: FunctionHeaderWithParameters) = this(s"${functionHeaderWithParameters.source()}")
  def source() = _source
}
...
class ParameterDeclaration private (_source: String) {
  ...
  def this(parameterQualifier: ParameterQualifier, parameterDeclarator: ParameterDeclarator) = this(s"${parameterQualifier.source()}${parameterDeclarator.source()}")
  def this(parameterTypeQualifier: ParameterTypeQualifier, parameterQualifier: ParameterQualifier, parameterTypeSpecifier: ParameterTypeSpecifier) = this(s"${parameterTypeQualifier.source()}${parameterQualifier.source()}${parameterTypeSpecifier.source()}")
  def this(parameterQualifier: ParameterQualifier, parameterTypeSpecifier: ParameterTypeSpecifier) = this(s"${parameterQualifier.source()}${parameterTypeSpecifier.source()}")
  def source() = _source
}
...
class InitDeclaratorList private (_source: String) {
  ...
  def this(initDeclaratorList: InitDeclaratorList, IDENTIFIER: Identifier) = this(s"${initDeclaratorList.source()}, ${IDENTIFIER.source()}")
  def this(initDeclaratorList: InitDeclaratorList, IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, constantExpression: ConstantExpression) = this(s"${initDeclaratorList.source()}, ${IDENTIFIER.source()}[ ${constantExpression.source()}] ")
  def this(initDeclaratorList: InitDeclaratorList, IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, initializer: Initializer) = this(s"${initDeclaratorList.source()}, ${IDENTIFIER.source()}[ ] = ${initializer.source()}")
  def this(initDeclaratorList: InitDeclaratorList, IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, constantExpression: ConstantExpression, initializer: Initializer) = this(s"${initDeclaratorList.source()}, ${IDENTIFIER.source()}[ ${constantExpression.source()}] = ${initializer.source()}")
...
class SingleDeclaration private (_source: String) {
  ...
  def this(fullySpecifiedType: FullySpecifiedType, IDENTIFIER: Identifier) = this(s"${fullySpecifiedType.source()}${IDENTIFIER.source()}")
  def this(fullySpecifiedType: FullySpecifiedType, IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, constantExpression: ConstantExpression) = this(s"${fullySpecifiedType.source()}${IDENTIFIER.source()}[ ${constantExpression.source()}] ")
  def this(fullySpecifiedType: FullySpecifiedType, IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, initializer: Initializer) = this(s"${fullySpecifiedType.source()}${IDENTIFIER.source()}[ ] = ${initializer.source()}")
  def this(fullySpecifiedType: FullySpecifiedType, IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, constantExpression: ConstantExpression, initializer: Initializer) = this(s"${fullySpecifiedType.source()}${IDENTIFIER.source()}[ ${constantExpression.source()}] = ${initializer.source()}")
...
class LayoutQualifierId private (_source: String) {
  ...
  def this(IDENTIFIER: Identifier, UINTCONSTANT: Uintconstant) = this(s"${IDENTIFIER.source()}= ${UINTCONSTANT.source()}")
  def source() = _source
}
...
class TypeQualifier private (_source: String) {
  ...
  def this(layoutQualifier: LayoutQualifier, storageQualifier: StorageQualifier) = this(s"${layoutQualifier.source()}${storageQualifier.source()}")
  def this(interpolationQualifier: InterpolationQualifier, storageQualifier: StorageQualifier) = this(s"${interpolationQualifier.source()}${storageQualifier.source()}")
  def this(interpolationQualifier: InterpolationQualifier) = this(s"${interpolationQualifier.source()}")
  def this(invariantQualifier: InvariantQualifier, storageQualifier: StorageQualifier) = this(s"${invariantQualifier.source()}${storageQualifier.source()}")
  def this(invariantQualifier: InvariantQualifier, interpolationQualifier: InterpolationQualifier, storageQualifier: StorageQualifier) = this(s"${invariantQualifier.source()}${interpolationQualifier.source()}${storageQualifier.source()}")
  def source() = _source
}
...
class TypeSpecifierNoPrec private (_source: String) {
  ...
  def this(typeSpecifierNonarray: TypeSpecifierNonarray, LEFT_BRACKET: LeftBracket) = this(s"${typeSpecifierNonarray.source()}[ ] ")
...
class StructDeclarationList private (_source: String) {
  ...
  def this(structDeclarationList: StructDeclarationList, structDeclaration: StructDeclaration) = this(s"${structDeclarationList.source()}${structDeclaration.source()}")
  def source() = _source
}
...
class StructDeclarator private (_source: String) {
  ...
  def this(IDENTIFIER: Identifier, LEFT_BRACKET: LeftBracket, constantExpression: ConstantExpression) = this(s"${IDENTIFIER.source()}[ ${constantExpression.source()}] ")
  def source() = _source
}
...
class IterationStatement private (_source: String) {
  ...
  def this(statementWithScope: StatementWithScope, expression: Expression) = this(s"do ${statementWithScope.source()}while ( ${expression.source()}) ; ")
  def this(forInitStatement: ForInitStatement, forRestStatement: ForRestStatement, statementNoNewScope: StatementNoNewScope) = this(s"for ( ${forInitStatement.source()}${forRestStatement.source()}) ${statementNoNewScope.source()}")
  def source() = _source
}
