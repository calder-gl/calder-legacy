...
class Shader(val main: Function = new Function("main"),
             ...
             val outputDeclarations: Array[VariableDeclaration] = Array()) extends SyntaxNode {
  def source(): String =
  ...
  def inputs(): Set[VariableSource] = inputDeclarations.map(declaration ⇒ declaration.variable.variableSource).toSet

  def outputs(): Set[VariableSource] = outputDeclarations.map(declaration ⇒ declaration.variable.variableSource).toSet
}
