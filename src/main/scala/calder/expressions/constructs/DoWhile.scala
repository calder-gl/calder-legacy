...
class DoWhile(override protected val condition: Expression, override protected val loopBlock: Block)
    extends While(condition, loopBlock) {
  override def source(): String =
    s"do ${loopBlock.source} while (${condition.source})"
}
